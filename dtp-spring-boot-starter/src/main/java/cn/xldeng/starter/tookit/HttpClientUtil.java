...
public class HttpClientUtil {
    ...
    @SneakyThrows
    public <T> T restApiGetByThreadPool(String url,
    ...
    @SneakyThrows
    public <T> T restApiPostByThreadPool(String url,
                                         Map<String, String> headers,
                                         Map<String, String> paramValues,
                                         Long readTimeout,
                                         Class<T> clazz) {
        ...
        Response response = call.execute();
        if (response.code() != HTTP_OK_CODE) {
            String msg = String.format("HttpPost 响应 code 异常. [code] %s [url] %s", response.code(), url);
            log.error(msg);
            throw new RuntimeException(msg);
        }
        assert response.body() != null;
